...
const Buy = () => {
  ...
  const handleSearch = async () => {
    if (searchQuery.trim()) {
      try {
        const response = await fetch(`http://localhost:8080/market-items/search?q=${encodeURIComponent(searchQuery)}`);
    ...
    <div className="items-grid">
      {searchResults.map(item => (
        ...
        style={{ cursor: "pointer" }}>
          ...
          <p>Added: {item.addedAt ? new Date(item.addedAt).toLocaleDateString() : 'N/A'}</p>
        </div>
      ))}
    </div>
  </>
