    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/api/login", "/register", "/profile", "/profile/update", "/market-items", "/market-items/{id}", "/market-items/category/{category}","/market-items/search","/transport/fetchdestination", "/transport/book", "/transport/daily-commute","/urgent","/resources/upload", "/resources/fetch-subjects","/resources/fetch","/resources/download/**","/resources/notify-me","/resources/notifications/fetch", "/transactions/buyer","/transactions/seller","/user/buysellprofile", "/transactions/request", "/transactions/**", "/counsellor/**", "/counsellingbook/**","/student/notification/**").permitAll()
                .anyRequest().authenticated()
            )
            .formLogin(form -> form.disable())
            .httpBasic(httpBasic -> httpBasic.disable())
